...
public class FundManagerSystem {
    ...
    private static void buyStock(String name) {
        Fund fund = null;
        try {
            ...
            while (true) {
                Scanner sc = new Scanner(System.in);
                System.out.println("請輸入股票名稱(或輸入n離開): ");
    ...
    private static void queryStocks() throws Exception {
        ...
        //System.out.println(obj.writeValueAsString(stocks)); //此方式表現不出 fetch型態為lazy時的樣式
    ...
    private static void queryFunds() throws Exception {
        ...
        //System.out.println(obj.writeValueAsString(funds));    //此方式表現不出 fetch型態為lazy時的樣式
        //System.out.println(new Gson().toJson(funds)); //此方式太難看
